...
void *_malloc(uint nbytes) {
    ...
    while(currentBlock->nextBlock != NULL && (currentBlock->size - nbytes) >= (BLOCK_SIZE + forceAlign)) {
        if(currentBlock->size == nbytes && currentBlock->available == TRUE) {
            ...
            freeList = currentBlock;
            currentBlock = currentBlock->nextBlock; // CORRECTION: adding this line to fix the infinite loop
        }
    }
...
void _free(void *ptr) {
    ...
    // searching through blocks for pointer matching the one that was passed to free
    current = &base;
    while(current != NULL) {
        if(new == current) { // CORRECTION
            current->available = TRUE;
            current->size = 0;
            freeList = current;
        }
        current = current->nextBlock; // CORRECTION: moving this line outside the if-statement to fix the infinite loop
    }
